...
ABLATIONS: List[AblationSpec] = [
    ...
    AblationSpec(
        code="A2_NO_WEIGHT",
        description="remove lc_weight only",
        weight_col=None,
    ),
    AblationSpec(
        code="A3_NO_FRAC_ENTROPY",
        description="remove lc_dom_frac and lc_entropy",
        extra_num_cols=tuple(),
    ),
    AblationSpec(
        code="A4_NO_FRAC",
        description="remove lc_dom_frac only",
        extra_num_cols=("lc_entropy",),
    ),
    AblationSpec(
        code="A5_NO_ENTROPY",
        description="remove lc_entropy only",
        extra_num_cols=("lc_dom_frac",),
    ),
    AblationSpec(
        code="A6_NO_VOD",
        description="remove all VOD features",
        include_vod=False,
    ),
    AblationSpec(
        code="A7_NO_CORE",
        description="remove LAI/Hveg/SM/LST",
        include_core=False,
    ),
    AblationSpec(
        code="A8_NO_SEASON",
        description="remove doy_sin/doy_cos/lat_norm",
        include_season=False,
    ),
    AblationSpec(
        code="A9_NO_IGBP_PROPS",
        description="remove IGBP_prop_* numeric features",
        include_igbp_props=False,
    ),
]


# =======================
# Ablation-specific dataset
# =======================
def build_ablation_frame(
