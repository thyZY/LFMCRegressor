...
from lfmc_final_temporal_cnn_common import DualBranchTemporalCNNRegressor, LabelEncoderWithUNK
...
def predict_in_batches(
    model: DualBranchTemporalCNNRegressor,
    x_static: np.ndarray,
    x_cat: np.ndarray,
    x_dyn: np.ndarray,
    device: torch.device,
...
def run_predict_raster(
    ...
    model, cfg, encoder = load_temporal_cnn_artifact(artifact_dir, device)
    ...
    print(f"[OK] Temporal raster prediction finished -> {output_path}")
