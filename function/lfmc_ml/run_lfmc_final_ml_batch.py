...
def _encode_category(train_df, val_df, test_df, cat_col):
    all_vals = (
        np.concatenate(
            [
                train_df[cat_col].astype(str).to_numpy(),
                val_df[cat_col].astype(str).to_numpy(),
                test_df[cat_col].astype(str).to_numpy(),
            ]
        )
    )
...
def main():
    ...
    for rep in range(args.repeats):
        ...
        for model_name, preds in preds_by_model.items():
            print(f"[repeat {rep + 1}/{args.repeats}] finished {model_name}")
            metrics_by_split = {}
            ...
            print_split_metrics(model_name, rep + 1, metrics_by_split)

    raw_path, summary_path = save_result_tables(rows, OUT_DIR, "lfmc_final_s6_lite_ml")
