...
def main():
    ...
    for spec in ABLATIONS_V2:
        print(f"\n=== Running {spec.code} | {spec.description} ===")

        ab_df, num_cols, cat_col = build_ablation_v2_frame(base_df, spec, igbp_num_cols)
        ...
        print(f"weight min/max: {train_df['sample_weight'].min():.4f}, {train_df['sample_weight'].max():.4f}")
    ...
    raw_path, summary_path = save_result_tables(rows, OUT_DIR, "lfmc_s6_catboost_ablation_v2")
